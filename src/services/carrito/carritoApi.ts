...
export async function addPaqueteEventoCarrito(paqueteId: string) { const r = await apiClient.post(`/Carrito/items/paquete-evento/${paqueteId}`); return r.data; }
export async function addFotoPrivadaCarrito(fotoPrivadaId: string) { const r = await apiClient.post(`/Carrito/items/foto-privada/${fotoPrivadaId}`); return r.data; }
