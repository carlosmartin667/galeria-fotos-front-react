...
export function AdminClienteHistorialPage(){const{id=''}=useParams();const q=useQuery({queryKey:['admin','cliente-historial',id],queryFn:()=>getClienteHistorialAdmin(id)});return <section><AdminPageHeader title="Historial cliente"/>{(q.data??[]).map((item,i)=><div className="card mb-2" key={i}><div className="card-body"><SanitizedMetadata value={item}/></div></div>)}<NotasInternasPanel entidadTipo="Cliente" entidadId={id}/></section>}
